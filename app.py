...
def main():
    ...
    with st.container():
        ...
        # Test URLs section (placed below the main info)
        st.markdown("---")
        st.subheader("Test Examples")
        st.markdown("Click on an example to load it into the scanner.")
        
        test_urls = {
            "Safe (Google)": "https://www.google.com/search?q=hello",
            "Phishing (IP Address)": "http://192.168.1.1/login.php",
            "Suspicious (Long Domain)": "https://verify-account-security-update-portal-login.com",
            "Shortened URL": "http://bit.ly/3m4aP1p"
        }
        
        test_cols_btn = st.columns(4) # Changed to 4 columns to fit better
        for i, (desc, url) in enumerate(test_urls.items()):
            with test_cols_btn[i % 4]:
                if st.button(desc, key=f"test_btn_{i}", use_container_width=True):
                    st.session_state['url_input_field'] = url
                    st.rerun()
